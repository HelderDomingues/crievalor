...
function SplashCursor({
  ...
  // Using primary blue color from the theme
  ...
  useEffect(() => {
    ...
    function updateColors(dt: number) {
      ...
      if (colorUpdateTimer >= 1) {
        ...
        pointers.forEach((p) => {
          p.color = generateBrandColor(); // Use brand colors
        });
      }
    }
    ...
    function clickSplat(pointer: Pointer) {
      // Use the brand colors - blue/purple tones
      const color = generateBrandColor();
      color.r *= 10.0;
      color.g *= 10.0;
      color.b *= 10.0;
    ...
    function updatePointerDownData(pointer: Pointer, id: number, posX: number, posY: number) {
      ...
      pointer.deltaX = 0;
      pointer.deltaY = 0;
      pointer.color = generateBrandColor(); // Use brand colors
    }
    ...
    function updatePointerMoveData(pointer: Pointer, posX: number, posY: number, color: {r: number, g: number, b: number}) {
      ...
      pointer.color = color || generateBrandColor();
    }
    ...
    function generateBrandColor(): {r: number, g: number, b: number} {
      // Blue to purple spectrum (0.5-0.8 in HSV hue)
      const hue = 0.6 + Math.random() * 0.2; // Range from ~blue to ~purple
      const saturation = 0.7 + Math.random() * 0.3; // Medium to high saturation
      const value = 0.5 + Math.random() * 0.5; // Medium to high brightness
      
      let c = HSVtoRGB(hue, saturation, value);
      c.r *= 0.2;
      c.g *= 0.1;
      c.b *= 0.4; // Emphasize blue/purple
      return c;
    }
    ...
    document.body.addEventListener(
      ...
      function handleFirstMouseMove(e) {
        ...
        let posX = scaleByPixelRatio(e.clientX);
        let posY = scaleByPixelRatio(e.clientY);
        let color = generateBrandColor(); // Use brand colors
