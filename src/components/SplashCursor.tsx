...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function simulate(time: number) {
      const dt = Math.min(
        (time - (lastUpdateTime || time)) / 1000,
        0.016
      );
      lastUpdateTime = time;

      if (pointers[0].down && !config.PAUSED) {
      ...
      if (
        !config.PAUSED &&
        (pointers[0].moved || lastColorChangeTime + 100 < Date.now())
      ) {
        pointers[0].moved = false;
        lastColorChangeTime = Date.now();

        multipleSplats(parseInt(Math.random() * 20 + 5 + "", 10));
      }
