...
const MaritimeWaves: React.FC<MaritimeWavesProps> = ({ className = "" }) => {
  ...
  const drawWaves = (
    ...
    for (let x = 0; x <= width; x += 10) {
      const y = 
        height - 
        waveHeight - 
        Math.sin(x * 0.01 + time + offset) * amplitude * waveHeight - 
        Math.sin(x * 0.02 + time * 1.5) * (amplitude * 0.5) * waveHeight;
