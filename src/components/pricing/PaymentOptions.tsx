...
const PaymentOptions = ({ 
  ...
  const handlePaymentTypeChange = (value: string) => {
    ...
    // Always use installment 1 now (simplification)
  ...
  return (
    <div className="mt-8 bg-card border border-border rounded-xl p-6 md:p-8 max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        <div className="bg-secondary/10 p-4 rounded-lg">
          <div className="flex items-center mb-3">
            {paymentType === "credit" ? (
              <CreditCard className="text-primary h-6 w-6 mr-2" />
            ) : (
              <BadgePercent className="text-green-500 h-6 w-6 mr-2" />
            )}
            <h4 className="font-semibold">
              {paymentType === "credit" ? "Cartão de crédito" : "Pagamento à vista"}
            </h4>
          </div>
