...
const ApresentacaoSCPage = () => {
  ...
  // Slide 10 - Fechamento CTA (ALTERADO: Telefone SC)
  {
    id: 'fechamento',
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-8">
                <div className="flex items-center justify-center space-x-4">
                  ...
                  <span className="text-3xl font-semibold">(47) 99215-0289</span>
                </div>
                <div className="flex items-center justify-center space-x-4">
...
return <>
      <Helmet>
        <title>Apresentação Institucional SC - Crie Valor Consultoria</title>
        <meta name="description" content="Apresentação institucional da Crie Valor para Santa Catarina. Conheça nossos serviços, projetos e soluções em inteligência organizacional com IA." />
        <link rel="canonical" href="https://crievalor.com.br/apresentacao-sc" />
      </Helmet>
      ...
      <div className="relative z-10 h-screen w-screen overflow-hidden">
        ...
        align: "start"
          ...
          <div className="absolute bottom-12 left-1/2 transform -translate-x-1/2 flex items-center space-x-8 z-20">
            <CarouselPrevious className="relative static transform-none bg-primary/20 hover:bg-primary/40 border-primary/50 backdrop-blur-sm">
              <ChevronLeft className="h-6 w-6" />
            </CarouselPrevious>
            
            <div className="flex items-center space-x-3 bg-card/30 backdrop-blur-sm px-6 py-3 rounded-full border border-border/50">
              {Array.from({
            length: count
          }).map((_, index) => <div key={index} className={`h-2 w-2 rounded-full transition-all duration-300 ${current === index + 1 ? 'bg-primary w-8' : 'bg-muted-foreground/50'}`} />)}
            </div>

            <CarouselNext className="relative static transform-none bg-primary/20 hover:bg-primary/40 border-primary/50 backdrop-blur-sm">
              <ChevronRight className="h-6 w-6" />
            </CarouselNext>
          </div>

          {/* Slide Counter */}
          <div className="absolute top-8 right-8 bg-card/30 backdrop-blur-sm px-6 py-3 rounded-full border border-border/50 z-20">
        ...
        }} className="absolute top-8 left-8 bg-card/30 backdrop-blur-sm border border-border/50 hover:bg-card/50 z-20">
            <ExternalLink className="h-5 w-5" />
          </Button>
        </Carousel>
      </div>

      {/* Hide Header/Footer/Cookie Consent */}
      <style>{`
        header, footer, .cookie-consent, [data-whatsapp-widget] {
...
export default ApresentacaoSCPage;